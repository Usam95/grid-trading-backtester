...
- [ ] Universe construction excludes pegs, leveraged products, duplicates, unsupported histories, and venue-invalid symbols using predeclared rules rather than observed strategy returns.
- [ ] The liquidity rank uses only the accepted development-period statistic and produces a deterministic, reviewable five-symbol selection.
- [ ] Production history—not the current Testnet symbol set—defines eligibility and robustness membership.
- [ ] A coverage/size preview runs before bulk acquisition and avoids downloading five years of minute data for every production symbol.
